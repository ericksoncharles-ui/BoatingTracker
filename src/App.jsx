  ...
  return (
    <div className="app">
      ...
      <div className="tab-bar desktop-only">
        ...
        <button
          className={`tab-btn ${activeTab === 'navigate' ? 'tab-active' : ''}`}
          onClick={() => setActiveTab('navigate')}
        >
          <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="12 2 19 21 12 17 5 21"/></svg>
          Navigate
        </button>
      </div>
      ...
      <div className="mobile-layout mobile-only">
        ...
        <div
          ref={sheetRef}
          className={`mobile-sheet ${sheetOpen ? 'sheet-open' : 'sheet-collapsed'} ${activeTab === 'chart' || activeTab === 'navigate' ? 'sheet-hidden' : ''}`}
        ...
        <nav className="mobile-tab-bar">
          ...
          <button
            className={`mobile-tab ${activeTab === 'navigate' ? 'mobile-tab-active' : ''}`}
            onClick={() => { setActiveTab('navigate'); setSheetOpen(false) }}
          >
            <svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="12 2 19 21 12 17 5 21"/></svg>
            <span>Navigate</span>
          </button>
        </nav>
      </div>
